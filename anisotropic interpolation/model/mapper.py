...
class MapperNoEncoding(nn.Module):
  def __init__(self, factor= torch.pi):
    self.factor = factor
    super(MapperNoEncoding, self).__init__()
  def forward(self, x):
    return x * self.factor


# from official FF repository
class MapperFF(nn.Module):
  '''
  adapted from: https://github.com/tancik/fourier-feature-networks/tree/master
  '''
  def __init__(self, B=None, factor=1.0):
    super(MapperFF, self).__init__()
    self.B = factor * B
