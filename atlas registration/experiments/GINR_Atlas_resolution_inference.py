...
# -*- coding: utf-8 -*-
"""
Script to run image resolution experiment.
...
def main(hparams):
    ...
    sampling = config['TEST']["SUBSMPL_FACTOR"] = hparams.dres


    # set project
    if not hparams.justAtlas:
        project = f"{config['SETTINGS']['PROJECT_NAME']}_evaluation_resolution"
    else:
        project = f"{config['SETTINGS']['PROJECT_NAME']}_evaluation_atlas_only_resolution"
    ...
    # define run name
    run = "run_{}_FOLD_{}_SUBSMPL_{}".format(datetime.now().strftime("%d_%m_%Y_%H_%M_%S"), config['SETTINGS']['FOLD'], sampling)
    ...
    print(f"Sampling factor: {sampling}")
    print(f"Should be running on device: {hparams.device}")
    print("==================================")


    paths = compute_paths(data_path=config['SETTINGS']['DATA_DIR'] ,patients_list=folds['FOLD'][config['SETTINGS']['FOLD']]['TEST'][-2:], subsmpl_factor=sampling)
    ...
    for pat in paths:
        ...
        # varying patch size on image resolution
        if  sampling <= 8:
            pdres = 64
        else:
            pdres = 512//sampling
        ...
        dataset = HealthyOctDataset([pat], mode = config['DATA']['SAMPLE_MODE'], patch_size=(231, 62, pdres))
        ...
        model = GINRAtlasReg.load_from_checkpoint(model_checkpoint, deformation_model=displacement_model, atlas_model=atlas_model, map_location="cpu") ### !!!! IMPORTANT:  map_location="cpu" Otherwise can lead to error where the model don't train!
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dres", default=8, type=int, choices=[1,8,16,32], help="Resolution sampling of the volumes.")
