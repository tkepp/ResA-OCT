...
# -*- coding: utf-8 -*-
"""
Script to run the inference evaluation for affine registration.

"""
import os

from scripts.routines.Affine_Atlas_Reg_Inference import AffineAtlasRegInference
import time
...
from datasets.healthyOCTPatches import HealthyOctDatasetPatches
...
def manual_test(batch, model, atlas_only):
    ...
    if atlas_only:
        ...
        deformation = model.apply_affine(coords)
    ...
    metrics["neg_jacobian_metric"] = torch.tensor(-1)

    metrics["masked_neg_jacobian_metric"] =  torch.tensor(-1)

    if atlas_only:
        metrics["l1_deformation"] = torch.tensor(0.)
    else:
        metrics["l1_deformation"] = torch.tensor(-1)
...
def main(hparams):
    ...
    if not hparams.justAtlas:
        project = f"{config['SETTINGS']['PROJECT_NAME']}_affine"
    else:
