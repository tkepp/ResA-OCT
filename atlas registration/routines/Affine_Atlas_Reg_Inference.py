...
class AffineAtlasRegInference(L.LightningModule):

    def __init__(self, lr=1e-4, lr_exp_gamma=0.99, atlas_model=None, save_path=None):
        ...
        self.A = nn.Parameter(torch.eye(3, dtype=torch.float32)).to(self.device)
        self.t = nn.Parameter(torch.zeros(3, dtype=torch.float32)).to(self.device)

        self.save_hyperparameters(ignore=['atlas_model'])

    def apply_affine(self, coords):
        return coords @ self.A.T + self.t
    ...
    def training_step(self, batch, batch_idx):
        ...
        deformation = self.apply_affine(coords)
        ...
        reg_loss =  0.01 * (self.l1_loss(self.A, torch.eye(3, dtype=torch.float32).to(self.device)) + self.l1_loss(self.t, torch.zeros(3, dtype=torch.float32).to(self.device)))
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            [{"params": [self.A, self.t], "lr": self.lr}]
        )
